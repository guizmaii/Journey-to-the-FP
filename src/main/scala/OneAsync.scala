...
object OneAsync extends App {
  ...
  def xSecondsOfWorks(x: Int)(i: Int): Future[Unit] = {
    ...
    Future {
      ...
      println(s"After : ${Thread.currentThread().getName} - $i - Time passed: $millisPassed")
    }
  }
  ...
  val done = work().map(_ => {
    val after = System.currentTimeMillis()
    val millisPassed: Duration = (after.milliseconds - before.milliseconds).toSeconds.seconds
    println(s"AFTER ALL : ${Thread.currentThread().getName} - Total time passed: $millisPassed")
  })
